...
def query_mime_for_extension(index_path: Path, extension: str, limit: int) -> list[tuple]:
    """Query the index for MIME types associated with an extension."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s pdf                    # Find MIME types for .pdf files
  %(prog)s --index ./cc-index/CC-MAIN-2024-51/ png
  %(prog)s qoi --limit 20         # Show top 20 MIME types
""",
    )
    ...
    parser.add_argument(
        "--index", "-i",
        default="./cc-index/CC-MAIN-2024-51/",
        help="Path to Common Crawl parquet index (default: ./cc-index/CC-MAIN-2024-51/)",
    )
    parser.add_argument(
        "--limit", "-n",
        type=int,
    ...
    try:
        results = query_mime_for_extension(index_path, ext, args.limit)
        ...
        if top_mime in ("text/html", "application/xhtml+xml"):
            ...
            for mime, count in results:
                if mime not in ("text/html", "application/xhtml+xml"):
                    ...
                    print(f"\n  ./cc_download.py --local-index {args.index} --mime '{mime}' \\")
                    print(f"      --file-format {ext} --output-dir corpus/")
                    break
            else:
                print(f"  Warning: .{ext} URLs mostly serve HTML - may not contain actual {ext} files.")
        else:
            print(f"  ./cc_download.py --local-index {args.index} --mime '{top_mime}' \\")
            print(f"      --file-format {ext} --output-dir corpus/")
